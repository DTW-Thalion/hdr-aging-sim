"""
Test B: Antidiabetics and D-vs-J discrimination.

HDR prediction: metformin/sulfonylureas/insulin primarily target τ_M (D-matrix).
  - M→I (metabolic drive on inflammation): should WEAKEN in antidiabetic users
  - I→M (inflammatory drive on metabolism): should NOT change (J-matrix intact)
Asymmetric attenuation = D-vs-J decomposition validated.
"""
...
def run():
    panel, _ref = load_panel_with_deltas()
    ...
    for direction in ["I_to_M", "M_to_I"]:
        ...
        for gkey, gname in [(0, "nonuser"), (1, "antidm")]:
            sub = matched[matched["group"] == gkey]
            if len(sub) >= 30:
    ...
    # D-vs-J test: is M→I interaction p SIGNIFICANT while I→M interaction p is NOT?
...
def _figure(result):
    ...
    ax.set_title(f"Test B — Antidiabetics: D-vs-J discrimination — {status}",
                 fontsize=11)
    fig.tight_layout()
...
def _print_summary(r):
    print("=" * 66)
    print("TEST B — ANTIDIABETICS AND D-vs-J DISCRIMINATION")
    print("=" * 66)
    for direction in ["I_to_M", "M_to_I"]:
